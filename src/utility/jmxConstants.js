...
export const resolveArray = async (myArray, request_id) => {
    ...
    let toSend = ''
    if (Object.keys(myArray[0])[0].includes('$#$')) {
        let obj = {}
        myArray.forEach(o => {
            const key = Object.keys(o)[0];
            const value = o[key];
            const allKeys = key.split('$#$');
            _.set(obj, allKeys.join('.'), value);
        })
        toSend = `<boolProp name="HTTPSampler.postBodyRaw">true</boolProp>
        <elementProp name="HTTPsampler.Arguments" elementType="Arguments">
          <collectionProp name="Arguments.arguments">
            <elementProp name="" elementType="HTTPArgument">
              <boolProp name="HTTPArgument.always_encode">false</boolProp>
              <stringProp name="Argument.value">{${JSON.stringify(obj).replace(/"/g, '&quot;')}}</stringProp>
